...
import (
	...
	"github.com/yury-sannikov/amneziawg-go/conn/bindtest"
	"github.com/yury-sannikov/amneziawg-go/tun"
	"github.com/yury-sannikov/amneziawg-go/tun/tuntest"
)
...
func genTestPair(
	...
	for i := range pair {
		...
		bind := binds[i]
		p.dev = NewDevice(p.tun.TUN(), bind, func() conn.Bind { return bind }, NewLogger(level, fmt.Sprintf("dev%d: ", i)))
