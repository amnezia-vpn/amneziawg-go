...
func (device *Device) RoutineReceiveIncoming(
	...
	for {
		...
		for i, size := range sizes[:count] {
			...
			// decode D1-D4
			deobfBuf := device.GetMessageBuffer()
			n, _ := device.DeobfuscatePacket(deobfBuf[:], packet)
			if n > 0 {
				device.PutMessageBuffer(bufsArrs[i])
				bufsArrs[i] = deobfBuf
				packet = deobfBuf[:n]
			} else {
				device.PutMessageBuffer(deobfBuf)
			}
...
func (device *Device) DeobfuscatePacket(dst, src []byte) (n int, expectedType uint32) {
	if chain := device.dpackets.init; chain != nil && chain.Deobfuscate(dst, src) {
		return chain.DeobfuscatedLen(len(src)), MessageInitiationType
	}

	if chain := device.dpackets.response; chain != nil && chain.Deobfuscate(dst, src) {
		return chain.DeobfuscatedLen(len(src)), MessageResponseType
	}

	if chain := device.dpackets.cookie; chain != nil && chain.Deobfuscate(dst, src) {
		return chain.DeobfuscatedLen(len(src)), MessageCookieReplyType
	}

	if chain := device.dpackets.transport; chain != nil && chain.Deobfuscate(dst, src) {
		return chain.DeobfuscatedLen(len(src)), MessageTransportType
	}

	return 0, MessageUnknownType
}
